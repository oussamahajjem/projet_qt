...
{
    Id= 0;nomPrenom=" ";hebergement=" ";prix=0;codeC=" ";

}
reservationh::reservationh(int Id,QString nomPrenom,QString hebergement,QDate checkin,QDate checkout,int prix,QString codeC)
{
    this->Id=Id;this->nomPrenom=nomPrenom;this->hebergement=hebergement;this->checkin=checkin;this->checkout=checkout;this->prix=prix;this->codeC=codeC;
}
...
QString reservationh::getcodeC(){return codeC;}
...
void reservationh::setcodeC(QString codeC){this->codeC=codeC;}
bool reservationh::ajouter()
{
    QSqlQuery query;
    QString Id_string=QString::number(Id);
    QString prix_string=QString::number(prix);
          query.prepare("INSERT INTO RESERVATIONH (Id,nomPrenom,hebergement,checkin,checkout,prix,codeC) "
                        "VALUES (:Id,:nomPrenom,:hebergement,:checkin,:checkout,:prix,:codeC)");
...
QSqlQueryModel* reservationh::afficher()
{
   ...
   /* QSqlQuery test*/
    ...
    model->setHeaderData(6,Qt::Horizontal,QObject::tr("codeC"));
    return model;
...
bool reservationh::modifier(int Id)
{
    ...
    QString Id_string=QString::number(Id);
    QString prix_string=QString::number(prix);
    query.prepare("UPDATE RESERVATIONH SET Id=:Id,nomPrenom=:nomPrenom,hebergement=:hebergement,checkin=:checkin,checkout=:checkout,prix=:prix,codeC=:codeC WHERE Id=:Id");
    query.bindValue(":Id",Id_string);
    query.bindValue(":nomPrenom",nomPrenom);
    query.bindValue(":hebergement",hebergement);
    query.bindValue(":checkin",checkin);
...
QSqlQueryModel* reservationh::rechercheC(QString a)
{


    QSqlQueryModel *model=new QSqlQueryModel();
        model->setQuery("SELECT * FROM RESERVATIONH WHERE ( coddeC LIKE '%"+a+"%'  ) ");
            model->setHeaderData(0,Qt::Horizontal,QObject::tr("Id"));
            model->setHeaderData(1,Qt::Horizontal,QObject::tr("nomPrenom"));
            model->setHeaderData(2,Qt::Horizontal,QObject::tr("hebergement"));
            model->setHeaderData(3,Qt::Horizontal,QObject::tr("checkin"));
...
QSqlQueryModel*  reservationh::alerte()
{
    ...
    QSqlQueryModel *model=new QSqlQueryModel();
             model->setQuery("SELECT Id,nomPrenom,hebergement,checkin,checkout, 0.8*prix,codeC FROM RESERVATIONH WHERE (PRIX>=1500) ");
